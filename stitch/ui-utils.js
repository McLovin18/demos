...
import { getCurrentUser, logoutUser, CATEGORIES } from './users-data.js';
...
export const renderCategories = (containerId) => {
  ...
  container.innerHTML = CATEGORIES.map(
    (cat) => `
    <button onclick="toggleSubcategories('${cat.id}')" class="flex items-center justify-between w-full px-4 py-3 text-sm font-medium hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors rounded-lg">
      <span class="flex items-center gap-2">
        <span class="material-icons-round text-lg">${cat.icon}</span>
        ${cat.label}
      </span>
      <span class="material-icons-round text-lg md:hidden">keyboard_arrow_right</span>
    </button>
  `
  ).join('');
};
