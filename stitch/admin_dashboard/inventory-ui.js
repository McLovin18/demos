...
function loadProductsTable(productsToDisplay = null) {
  const products = productsToDisplay || getInventoryProducts();
    ...
    tableBody.innerHTML = `
      <tr class="border-b border-slate-100 dark:border-slate-700">
        <td colspan="6" class="px-6 py-8 text-center text-slate-500 dark:text-neutral-400">
          ...
          <p>No hay productos. Crea uno o migra desde products.js</p>
        </td>
      </tr>
    `;
    return;
  ...
  tableBody.innerHTML = products.map(product => {
    ...
    return `
      <tr class="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900/50">
        <td class="px-4 lg:px-6 py-4 text-xs lg:text-sm font-mono text-slate-500">#${product.id}</td>
...
function setupEventListeners() {
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      ...
      const results = query ? searchProducts(query) : getInventoryProducts();
      loadProductsTable(results);
    });
  }
}
...
// Guardar producto
function saveProduct(form, productId = null) {
...
// Eliminar producto
export function deleteProductHandler(productId) {
...
// Migrar productos
export async function migrateProductsDialog() {
  const response = await migrateProductsFromJS();
...
window.migrateProductsDialog = migrateProductsDialog;
